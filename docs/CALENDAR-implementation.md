...
4. Link `calendar_id` equals `CAL_DEV_CALENDAR_ID` **and** is listed in `S01_CONFIG.allowedCalendarIds`. Anything else → NeedsReview before any API call. An allowlisted non-DEV id is still refused.
...
- Every outcome writes an `AuditEvents` row (`executing_service: CalendarService`, `correlation_id: <outbox id>`): CalendarCreate/Update/Cancel, `…Retry`, `…Failed`, `CalendarStalled`, `CalendarReview<Resolution>`, `AssignDevCalendar`.
...
| `_calAssignDevCalendar(store, {actor, command_id, person_ids})` | Set `People.calendar_id` to the DEV calendar for explicitly named active Installers; audited; idempotent |
...
| `runCalAssignDevCalendar('PERSON-a,PERSON-b')` | Cloud DEV calendar assignment |
...
4. `runCalDispatchDryRun()` — preview S11/S15 rows; expect `NeedsReview:CALENDAR_TARGET_NOT_DEV` for fixture links pointing at `CAL-S11-CAPTURE` until installers are assigned the DEV calendar.
5. Set `calendarMode` to `LIVE`; `runCalLiveDevSmoke()` — expect pass, then check the DEV calendar is clean.
6. `runCalAssignDevCalendar('<installer person ids>')` for the real installer People rows, then `runCalEnableFn02ForSyntheticTest()` + `runCalDispatch()` for pilot jobs; `restoreCalSafeState()` afterwards.
...
- Installer `People.calendar_id` values in DEV are `NOT_CONFIGURED`/synthetic; assignment is a DEV data step via `runCalAssignDevCalendar`.
- Guests are not invited (S11 payload `guests: []`); per-installer calendars are out of scope for DEV.
